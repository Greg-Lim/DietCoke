...
class Knowledge():
    ...
    def knowledge_generate(model, test_data, caption_dict,syn_question_dict,syn_answer_dict,ans_to_cap_dicts, config):
        ...
        for n, per_test_data in enumerate(test_data):
            ...
            print(short_knowledge_messages)
    ...
    def knowledge_generate_single(model, question, ans_dict_queid, syn_question_queid, caption, syn_ans_queid, config):
        ...
        with futures.ThreadPoolExecutor() as executor:
            future_short = executor.submit(llm_generate, model=model, messages=short_knowledge_messages, temperature=0.7, max_tokens=1000)
            future_long = executor.submit(llm_generate, model=model, messages=long_knowledge_messages, temperature=0.7, max_tokens=1000)
...
class AnswerCandidates():
    ...
    def answer_candidates_generate_single(model, question, ans_dict_queid, syn_question_queid, caption, syn_ans_queid, config, long_knowledge, short_knowledge):
        ...
        with futures.ThreadPoolExecutor() as executor:
            future_long = executor.submit(llm_generate, model=model, messages=message_long, temperature=0, max_tokens=20)
            future_short = executor.submit(llm_generate, model=model, messages=message_short, temperature=0, max_tokens=20)
            future_cap = executor.submit(llm_generate, model=model, messages=message_cap, temperature=0, max_tokens=20)
...
class AutoRationales():
    ...
    def auto_rationale_generate_single(model, question, answer_long, answer_short, answer_cap, ans_dict_queid, syn_question_queid, caption, syn_ans_queid, config):
        ...
        with futures.ThreadPoolExecutor() as executor:
            future_long = executor.submit(llm_generate, model=model, messages=messages_long, temperature=0, max_tokens=1000)
            future_short = executor.submit(llm_generate, model=model, messages=messages_short, temperature=0, max_tokens=1000)
            future_cap = executor.submit(llm_generate, model=model, messages=messages_cap, temperature=0, max_tokens=1000)
...
class AnswerFusion():
    ...
    def answer_fusion_single(model, question, answer_cap, rationale_cap, answer_long, rationale_long, answer_short, rationale_short, ans_dict_queid, syn_question_queid, caption, syn_ans_queid, config):
        ...
        # question_id_next = test_data[(n + 1) % len(test_data)]['question_id'] # not needed (this uses a different question as a example)
        ans_dict_queid # generated by map_words_to_captions
        syn_question_queid = None # just for a input to use the old function
        # syn_question_queid_next = syn_question_dict[question_id_next] # not needed (this uses a different question as a example)
...
def diet_coke_e2e(model, img_model, image, question, config):
    captions = caption_gen.generate_multiple_captions(img_model, config["num_caps_per_img"], image, config)
    qa_pairs = qa_gen.generate_qa_pairs_oai_client(img_model, captions)
    ...
    long_knowledge, short_knowledge = Knowledge.knowledge_generate_single(model, question, ans_dict, syn_answer, config)
