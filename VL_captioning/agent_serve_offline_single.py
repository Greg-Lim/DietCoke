...
class Knowledge():
    ...
    def knowledge_generate_single(model, question, ans_dict_queid, syn_question_queid, caption, syn_ans_queid, config):
        """
        ...
        with futures.ThreadPoolExecutor() as executor:
            future_short = executor.submit(llm_generate, client=model, model=config["llm_model"], messages=short_knowledge_messages, temperature=0.7, max_tokens=1000)
            future_long = executor.submit(llm_generate, client=model, model=config["llm_model"], messages=long_knowledge_messages, temperature=0.7, max_tokens=1000)
...
class AnswerCandidates():
    ...
    def answer_candidates_generate_single(model, question, ans_dict_queid, syn_question_queid, caption, syn_ans_queid, config, long_knowledge, short_knowledge):
        """
        Parameters:
        model (object): The language model used to generate answers.
        ...
        with futures.ThreadPoolExecutor() as executor:
            future_long = executor.submit(llm_generate, client=model, model=config["llm_model"], messages=message_long, temperature=0, max_tokens=20)
            future_short = executor.submit(llm_generate, client=model, model=config["llm_model"], messages=message_short, temperature=0, max_tokens=20)
            future_cap = executor.submit(llm_generate, client=model, model=config["llm_model"], messages=message_cap, temperature=0, max_tokens=20)
...
class AutoRationales():
    ...
    def auto_rationale_generate_single(model, question, answer_long, answer_short, answer_cap, ans_dict_queid, syn_question_queid, caption, syn_ans_queid, config):
        ...
        with futures.ThreadPoolExecutor() as executor:
            future_long = executor.submit(llm_generate, client=model, model=config["llm_model"], messages=messages_long, temperature=0, max_tokens=1000)
            future_short = executor.submit(llm_generate, client=model, model=config["llm_model"], messages=messages_short, temperature=0, max_tokens=1000)
            future_cap = executor.submit(llm_generate, client=model, model=config["llm_model"], messages=messages_cap, temperature=0, max_tokens=1000)
...
class AnswerFusion():
    ...
    def answer_fusion_single(model, question, answer_cap, rationale_cap, answer_long, rationale_long, answer_short, rationale_short, ans_dict_queid, syn_question_queid, caption, syn_ans_queid, config):
        cur_answer_cap = answer_cap
        cur_answer_long = answer_long
        cur_answer_short = answer_short
        # answer_index += 1
        ...
        messages = [{'role':'user', 'content':Prompt}]
        pred_answer = llm_generate(client=model, model=config["llm_model"], messages=messages, temperature=0, max_tokens=1000)
        pred_answer = pred_answer.choices[0].message.content
        pred_answer = pred_answer.lower()
        pred_answer = post_process(pred_answer)
...
def diet_coke_e2e(model, img_model, image, question, config):
    captions = caption_gen._generate_multiple_captions_vllm(img_model, config["vlm_model"], image, config['num_caps_per_img'])
    # captions = [' this image shows the pitcher on the mound', ' This image shows the pitcher in action', ' This image shows the pitcher of the game', ' This image shows the pitcher in the process of throwing the ball', ' this image shows the pitcher in the process of throwing a pitch']
    print("captions: ", captions)
    qa_pairs = qa_gen.generate_qa_pairs_oai_client(model, captions)
    # this 2 lines may be wrong
    ...
    long_knowledge, short_knowledge = Knowledge.knowledge_generate_single(model, question, ans_dict_queid, syn_question, captions, syn_answer, config)#ans_dict, syn_answer, captions, syn_question, config)
    pred_answer_long, pred_answer_short, pred_answer_cap = AnswerCandidates.answer_candidates_generate_single(model, question, ans_dict_queid, syn_question, captions, syn_answer, config, long_knowledge, short_knowledge)
    rationale_long, rationale_short, rationale_cap = AutoRationales.auto_rationale_generate_single(model, question, pred_answer_long, pred_answer_short, pred_answer_cap, ans_dict_queid, syn_question, captions, syn_answer, config)
    final_answer = AnswerFusion.answer_fusion_single(model, question, pred_answer_cap, rationale_cap, pred_answer_long, rationale_long, pred_answer_short, rationale_short, ans_dict_queid, syn_question, captions, syn_answer, config)
    return final_answer
